...
class BaseBstrapQRDQN(BaseBstrapDQN):
  ...
  def _compute_z_variance(self, agent_net):
    z_mean  = tf.reduce_mean(agent_net, axis=-1)          # out: [None, n_heads, n_actions]
    center  = agent_net - tf.expand_dims(z_mean, axis=-1) # out: [None, n_heads, n_actions, N]
    z_var   = tf.reduce_mean(tf.square(center), axis=-1)  # out: [None, n_heads, n_actions]
